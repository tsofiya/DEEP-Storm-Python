...
def limit_gpu_memory(fraction, allow_growth=False):
    ...
    fraction is None or (np.isscalar(fraction) and 0 <= fraction <= 1) or _raise(
...
def export_SavedModel(model, outpath, meta={}, format='zip'):
    ...
    format in supported_formats or _raise(
        ValueError("Unsupported format '%s', must be one of %s." % (format, str(supported_formats))))
